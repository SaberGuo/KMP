...
namespace ParamedModule.Other
{
    ...
    /// 干泵DVR
    /// </summary>
    [Export("GXS", typeof(IParamedModule))]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class GXS : PartModulebase
    {
        public ParGXS par = new ParGXS();
        ...
        public override void CreateSub()
        {
            ...
            ExtrudeFeature box = CreateBox(BoxSketch, UsMM(par.Par.Length), UsMM(par.Par.Width), UsMM(par.Par.Height));
        ...
        private void CreateTopFlanch(Face plane)
        {
            ...
            ExtrudeFeature flanch = CreateTopCY(flanchConcaveEF, cir, UsMM(par.Par.TopFlanch.D2), UsMM(par.Par.TopFlanch.H));
            Face flanchEF = InventorTool.GetFirstFromIEnumerator<Face>(flanch.EndFaces.GetEnumerator());
            CreateFlanceScrew(flanchEF, cir, par.Par.TopFlanch.N, UsMM(par.Par.TopFlanch.C / 2), UsMM(par.Par.TopFlanch.D0 / 2), UsMM(par.Par.TopFlanch.H));
        }
        ...
        {
            WorkPlane ScreenPlane = Definition.WorkPlanes.AddByPlaneAndOffset(plane, -UsMM(par.Par.ScreenY), true);
            ...
            Point2d p2 = InventorTool.CreatePoint2d(-UsMM(par.Par.ScreenLength) + 2, UsMM(par.Par.ScreenWidth) - 2);
        ...
        private ExtrudeFeature CreateScreenSur(object plane)
        {
            ...
            SketchLine line1 = osketch.SketchLines.AddByTwoPoints(InventorTool.CreatePoint2d(UsMM(par.Par.ScreenX), UsMM(par.Par.Height)), InventorTool.CreatePoint2d(UsMM(par.Par.ScreenX + par.Par.ScreenLength), UsMM(par.Par.Height)));
            Point2d p1 = line1.StartSketchPoint.Geometry;
            Point2d p2 = line1.EndSketchPoint.Geometry;
            Point2d p3 = InventorTool.CreatePoint2d(p1.X - UsMM(par.Par.ScreenHeight1) * Math.Cos(85 * Math.PI / 180), p1.Y + UsMM(par.Par.ScreenHeight1) * Math.Sin(85 * Math.PI / 180));
            Point2d p4 = InventorTool.CreatePoint2d(p2.X - UsMM(par.Par.ScreenHeight2) * Math.Cos(85 * Math.PI / 180), p2.Y + UsMM(par.Par.ScreenHeight2) * Math.Sin(85 * Math.PI / 180));
            ...
            Profile pro = osketch.Profiles.AddForSolid();
            ExtrudeDefinition def = Definition.Features.ExtrudeFeatures.CreateExtrudeDefinition(pro, PartFeatureOperationEnum.kJoinOperation);
            def.SetDistanceExtent(UsMM(par.Par.ScreenWidth), PartFeatureExtentDirectionEnum.kNegativeExtentDirection);
            return Definition.Features.ExtrudeFeatures.Add(def);
        }
        #endregion
        #region 创建底部支撑
        ...
        private ExtrudeFeature CreateWheelSur1(Face plane, double SurWidth, double distance)
        {
            ...
            Profile pro = osketch.Profiles.AddForSolid();
            ExtrudeDefinition def = Definition.Features.ExtrudeFeatures.CreateExtrudeDefinition(pro, PartFeatureOperationEnum.kJoinOperation);
            def.SetDistanceExtent(UsMM(par.Par.BottomHeight * 2 / 3), PartFeatureExtentDirectionEnum.kPositiveExtentDirection);
            return Definition.Features.ExtrudeFeatures.Add(def);

        }
        private RevolveFeature CreateWheel1(Face plane, double surWidth)
        {
            double radius = par.Par.BottomHeight / 3;
        ...
        {
            PlanarSketch osketch = Definition.Sketches.Add(plane);
            osketch.SketchLines.AddAsTwoPointRectangle(InventorTool.CreatePoint2d(UsMM(par.Par.Length - distance - SurWidth), UsMM(distance)), InventorTool.CreatePoint2d(UsMM(par.Par.Length - distance), UsMM(distance + SurWidth)));
            Profile pro = osketch.Profiles.AddForSolid();
            ExtrudeDefinition def = Definition.Features.ExtrudeFeatures.CreateExtrudeDefinition(pro, PartFeatureOperationEnum.kJoinOperation);
        ...
        private ExtrudeFeature CreateWheelSur2Cy(Face plane, double SurWidth)
        {
            ...
            Profile pro = osketch.Profiles.AddForSolid();
            ExtrudeDefinition def = Definition.Features.ExtrudeFeatures.CreateExtrudeDefinition(pro, PartFeatureOperationEnum.kJoinOperation);
            def.SetDistanceExtent(UsMM(par.Par.BottomHeight * 2 / 3) - 0.5, PartFeatureExtentDirectionEnum.kPositiveExtentDirection);
            return Definition.Features.ExtrudeFeatures.Add(def);
        }
        ...
        {
            CreateFan(plane, par.Par.FanX, par.Par.FanY, par.Par.FanWidth, par.Par.FanWidth, 4);
            CreatePump(plane, par.Par.PumpX, par.Par.PumpY, par.Par.PumpLenght, par.Par.PumpWidth, par.Par.PumpHeight);
            //法兰接口
            ExtrudeFeature cyling = CreateHole(plane, par.Par.SideFlanchX, par.Par.SideFlanchY, par.Par.SideFlanch.D6, 5, 47);
            CreateHole(plane, par.Par.SideHoleX1 - par.Par.SideFlanchX, par.Par.SideHoleY1 - par.Par.SideFlanchY, par.Par.SideHoleDia, par.Par.SideHoleThinkness, 19);
            CreateHole(plane, par.Par.SideHoleX2 - par.Par.SideFlanchX, par.Par.SideHoleY2 - par.Par.SideFlanchY, par.Par.SideHoleDia, par.Par.SideHoleThinkness, 19);
            ...
            CreateValve(StartFace, plane, par.Par.ValveX - par.Par.SideFlanchX, par.Par.ValveY - par.Par.SideFlanchY, par.Par.ValveInDia, par.Par.ValveThinkness, 35, 26);
        }
        private void CreateFan(Face plane, double sideX, double sideY, double length, double width, double depth)
        {
            PlanarSketch osketch = Definition.Sketches.Add(plane);
            double y = par.Par.Height - sideY;
